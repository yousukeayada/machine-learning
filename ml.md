...
- 決定木モデルを複数利用し、それらの多数決をとり予測結果とする。
- バギングという手法を用いる。
  - Bootstrap Aggregating（ブートストラップ法）の略。
  - 学習データからランダムに抽出することで、内容の異なる学習データを複数つくる（水増しする）。
#### 決定木
https://qiita.com/3000manJPY/items/ef7495960f472ec14377
- 学習データを条件分岐によって分割していくことで、分類・
回帰問題を解く手法。
- 分割の際、**不純度**を利用する。よく使われる不純度の指標として以下の２つがある。
  - ジニ係数
  - エントロピー
##### ジニ係数（Gini’s Diversity Index）
$$I_G = 1 - \sum_{i=1}^{n}{p_i^2}$$
- $n$ ：ラベル数、 $p_i$ ：各ラベルの割合
- この指標を用いて決定木を構築するアルゴリズムを **CART** と呼ぶ。
- 2分木が生成される。
##### エントロピー（平均情報量）
$$I_H = \sum_{i=1}^{n}{p_i\log_2 \frac{1}{p_i}} = -\sum_{i=1}^{n}{p_i\log_2 p_i}$$
- $n$ ：ラベル数、 $p_i$ ：各ラベルの割合
- 対数の底は 2 でなくてもよい（ラベル数に合わせると計算しやすい）
- この指標を用いて決定木を構築するアルゴリズムを **C4.5** と呼ぶ。
  - 不純度をより下げる、すなわち（分岐前エントロピー）-（分岐後エントロピー）の値 $\Delta I$ を大きくするような属性で学習データを分ける。
$$\Delta I = I - \sum_{i=1}^{b}{\frac{n_i}{N} I_i}$$
  - $b$ ：分岐数（ジニの場合 2）
  - $N$ ：分岐前のデータ数、 $n_i$ ：分岐後のデータ数
  - $I_i$ ：各分岐先のエントロピー
